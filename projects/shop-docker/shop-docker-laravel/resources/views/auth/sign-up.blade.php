...
@section('content')
    <x-forms.auth-forms title="Регистрация" action="{{route('store')}}" method="POST">
        @csrf
        <x-forms.text-input name="name" type="text" placeholder="Name"></x-forms.text-input>
        @error('email')
        <x-forms.error>
            {{ $message }}
        </x-forms.error>
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                ...
                <div class="text-xxs md:text-xs"><a href="{{ route('login') }}" class="text-white hover:text-white/70 font-bold">Ввойти в аккаунт</a></div>
            </div>
        </x-slot:buttons>
    </x-forms.auth-forms>
@endsection
